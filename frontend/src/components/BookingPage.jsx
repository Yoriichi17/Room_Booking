...
const BookingPage = () => {
  ...
  useEffect(() => {
    const fetchRooms = async () => {
      try {
        const res = await axios.get("http://localhost:5000/api/room/fetch");
    ...
    const fetchBookings = async () => {
      ...
      try {
        const res = await axios.get(
          `http://localhost:5000/api/booking/fetch?date=${selectedDate}&roomId=${activeRoom._id}`
        );
  ...
  const handleCancelBooking = async () => {
    try {
      await axios.delete(
        `http://localhost:5000/api/booking/cancel/${slotInfo._id}`
      );
